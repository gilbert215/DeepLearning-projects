...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            ...
            self.convolutional_layers.append(Conv1d(in_channels, out_channels, kernel_size, strides[i]))
            self.convolutional_layers[i].conv1d_stride1.W = conv_weight_init_fn(out_channels, in_channels, kernel_size)
            self.convolutional_layers[i].conv1d_stride1.b = bias_init_fn(out_channels)
            out_width = (out_width - kernel_size) // strides[i] + 1
            in_channels = out_channels

        in_dim = num_channels[-1] * out_width
        self.flatten = Flatten()
        self.linear_layer = Linear(in_dim, num_linear_neurons)
        self.linear_layer.W = linear_weight_init_fn(num_linear_neurons, in_dim)
